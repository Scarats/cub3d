...
/*   Updated: 2026/01/22 11:58:47 by chboegne         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (p.y < img->h)
	{
		...
		while (p.x < img->w)
		{
			...
			if (!is_transparent(color))
				my_pixel_put(data->img_buff, (p.x - PADDING)
					+ data->sprite_start.x, (p.y - PADDING)
					+ data->sprite_start.y, color);
			p.x++;
		}
		p.y++;
	}
...
void	sprites_handler(t_sprite *s, t_data *data)
{
	...
	time_diff = (int64_t)(curr_time.tv_sec - data->last_frame_time.tv_sec)
		* 1000000L + (int64_t)(curr_time.tv_usec
			- data->last_frame_time.tv_usec);
